...
val theEnigma = machine_create ()

in (*in-of-local*)

implement
TextProcessing_initize
  () = machine_setup (theEnigma, "XYZ")
// end of [TextProcessing_initize]
...
| _ when islower(c) => let
    val n = c - 'a'
    val v = machine_encode_char (theEnigma, n)
  in
...
{
  val c = TextProcessing_char(str[$UN.cast{sizeLt(n)}(i)])
}
//
in
